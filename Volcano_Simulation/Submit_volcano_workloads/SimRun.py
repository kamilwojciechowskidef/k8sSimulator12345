...
def step(sim_base_url, conf_file_url, pods_result_url, jobs_result_url, figures_result_url, scheduler):
    ...
    with plt.style.context('default'):
                                        ...
                                        if job_completed_podnumber < int(task["Pod"]["metadata"]["labels"]["jobTaskNumber"]):
                                            if task["Pod"]["status"]["phase"] == "Succeeded":
                                                ...
                                                # print(taskName, ":", task["NodeName"], ",", create, ",", start, ",", end)
                                                      # print(taskName, ":", task["NodeName"], ",", create, ",", start, ",", end)
                                                if job_completed_podnumber == int(task["Pod"]["metadata"]["labels"].get("jobTaskNumber", 0))- int(task["Pod"]["metadata"]["labels"].get("terminationLimit", 0)): # … dalsza logika …
       ...
       # Zapisywanie wyników dotyczących pojedynczych podów
    with open(pod_result_1, 'w', encoding='utf-8') as file2:
        file2.write(f"{succeed_table}\n")
        file2.write(f"TotalTask: {len(allpodruntime)}。\n")
        avg_time = sum(allpodruntime) / len(allpodruntime)
        min_time = min(allpodruntime)
        max_time = max(allpodruntime)
        file2.write(
            f"Task average time：{avg_time:.2f}s，"
            f"Minimum time：{min_time:.2f}s，"
            f"Maximum time：{max_time:.2f}s。\n"
        )

    # Zapisywanie podsumowania dotyczącego zadań
    job_result_1 = os.path.join(jobs_result_url, 'coutJCT.md')
    with open(job_result_1, 'w', encoding='utf-8') as file3:
        file3.write(f"{JCT_table}\n")
        file3.write("Summary:\n")
        file3.write(f"TotalJob: {len(countJct)}。\n")
        avg_job = sum(countJct) / len(countJct)
        min_job = min(countJct)
        max_job = max(countJct)
        file3.write(
            f"Job average time：{avg_job:.2f}s，"
            f"Minimum time：{min_job:.2f}s，"
            f"Maximum time：{max_job:.2f}s。\n"
        )
        file3.write(f"Jobs MakeSpan is：{max_job:.2f}s。\n")
...
if __name__ == '__main__':
    ...
    for i in range(1):
        ...
        #               "GANG_DRF_LRP", "GANG_DRF_MRP", "GANG_DRF_BRA", "GANG_DRF_BINPACK","BF_PW", "FF_PW", "LJF_PW", "SJF_PW", "LLMF_PW","Default"]
        #schedulers = ["BF_PW", "FF_PW", "LJF_PW", "SJF_PW", "LLMF_PW"]
        #schedulers=["Default"]
        schedulers={"DRF_BRA","GANG_BRA","SLA_BRA","GANG_DRF_BRA"}
